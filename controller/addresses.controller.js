...
export const index = (req, res) => {
    mydb.query(
        "SELECT gn_id AS id, full_address AS address, village FROM addresses",
        (err, result) => {
            if (err) throw err; 
...
export const show = (req, res) => {
    mydb.query(
        "SELECT gn_id AS id, full_address AS address, village FROM addresses WHERE gn_id = ?",
        [req.params.id],
        (err, result) => {
            if (err) throw err;
...
export const update = (req, res) => {
    ...
    mydb.query("UPDATE addresses SET ? WHERE gn_id = ?", [address, req.params.id], (err) => {
        if (err) throw err;
        res.redirect("/api/addresses");
    });
};
...
export const destroyPage = (req, res) => {
    mydb.query("SELECT gn_id AS id, full_address AS address, village FROM addresses WHERE gn_id = ?", [req.params.id], (err, result) => {
        if (err) throw err;
        if (!result[0]) {
            return res.redirect("/api/addresses");
        }
        res.render("addresses/destroy", { address: result[0] });
    });
};
